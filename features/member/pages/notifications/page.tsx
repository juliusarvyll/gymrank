...
import { markMemberNotificationRead } from "@/app/member/notifications/actions";
...
async function MemberNotificationsContent() {
  const { supabase, user, gym } = await requireMemberGym("/login?next=/notifications");
          ...
          <Card key={notification.id} className="rounded-[32px] border-white/80 bg-white/92 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.06)] sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex flex-col items-start gap-3 sm:items-end">
                ...
                {!notification.read_at ? (
                  <form action={markMemberNotificationRead}>
