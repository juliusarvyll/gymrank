...
import {
  addMemberActivityComment,
  postMemberShoutout,
  toggleMemberActivityReaction,
...
async function MemberCommunityContent() {
  const { supabase, gym, user } = await requireMemberGym("/login?next=/community");
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      <Card className="rounded-[32px] border-white/80 bg-white/92 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.06)] sm:p-6">
        ...
        <form action={postMemberShoutout} className="mt-5 space-y-3">
          ...
          return (
            <Card key={item.id} className="rounded-[32px] border-white/80 bg-white/92 p-5 shadow-[0_20px_60px_rgba(15,23,42,0.06)] sm:p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <form action={toggleMemberActivityReaction}>
              ...
              <form action={addMemberActivityComment} className="mt-4 space-y-3">
                <input type="hidden" name="activity_event_id" value={item.id} />
                <textarea
                  name="body"
                  rows={2}
