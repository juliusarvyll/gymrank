...
async function MemberCheckInContent() {
  const { supabase, user, gym } = await requireMemberGym("/login?next=/check-in");
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <Card className="overflow-hidden rounded-[32px] border border-emerald-200 bg-emerald-900 p-6 text-white shadow-sm sm:p-8">
          <div className="space-y-5">
            <Badge className="w-fit rounded-full border-none bg-white/14 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.28em] text-emerald-100">
              Check-In Lane
            </Badge>
            <div>
              ...
              <p className="mt-3 max-w-xl text-sm leading-6 text-emerald-50/85 sm:text-base">
                Check-ins happen through your QR pass. Keep it ready when you arrive at the gym.
              </p>
            </div>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
        <Card className="rounded-[32px] border-emerald-100 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-5 space-y-3">
            {checkins?.map((checkin) => (
              <div
                key={checkin.id}
                className="flex flex-col gap-3 rounded-[24px] border border-emerald-100 bg-emerald-50/70 p-4 sm:flex-row sm:items-center sm:justify-between"
              >
                <div>
            ...
            {!checkins?.length ? (
              <div className="rounded-[24px] border border-dashed border-emerald-200 bg-emerald-50/70 p-5 text-sm text-slate-500">
                No visits logged yet. Open your QR pass when you arrive at {gym.name}.
              </div>
            ) : null}
          </div>
        </Card>
        ...
        <Card className="rounded-[32px] border-emerald-100 bg-white p-5 shadow-sm sm:p-6">
          ...
          <div className="mt-5 rounded-[24px] bg-emerald-900 p-5 text-white">
